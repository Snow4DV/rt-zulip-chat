...
internal class ReactionView @JvmOverloads constructor(
    ...
    var emoji: Emoji
        get() = emojiMap[emojiCode] ?: error("Unable to get emoji: wrong emoji code was set to view")
        set(value) {
            emojiCode = value.code
        }
    var reaction: ChatReaction
        set(value) {
            emojiCode = value.emoji.code
            isSelected = value.userReacted
            count = value.count
            requestLayout()
        }
        get() = ChatReaction(emoji, count, isSelected)
