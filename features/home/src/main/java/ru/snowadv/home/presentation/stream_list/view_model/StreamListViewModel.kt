...
import ru.snowadv.home.data.StubStreamRepository
import ru.snowadv.home.data.StubTopicRepository
...
import ru.snowadv.home.presentation.util.toUiModelListWithPositions
...
internal class StreamListViewModel(
    ...
    private fun loadOrHideTopics(streamId: Long) {
        if (state.value.selectedStreamId == streamId) {
            ...
            when (resource) {
                is Resource.Error -> {
                    ...
                    viewModelScope.launch {
                        _eventFlow.emit(StreamListFragmentEvent.ShowInternetErrorWithRetry(
                            retryAction = {
